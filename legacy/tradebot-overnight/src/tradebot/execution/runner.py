...
# The SPY slice belongs entirely to the daily overnight rule (execution/overnight_runner.py).
# This loop neither targets it nor counts it in positions, so it can never buy or sell SPY.
OVERNIGHT_MANAGED = frozenset({"SPY"})


def build_strategy() -> ConstantWeightStrategy:
    # TODO: load target weights from config/DB instead of hardcoding once there's more
    # than one strategy in play.
    return ConstantWeightStrategy(weights=blended_weights())
...
def run_once(
    ...
    positions = [p for p in broker.positions() if p.symbol not in OVERNIGHT_MANAGED]
    ctx = StrategyContext(positions=positions, nav=nav)
    weights = {
        s: w
        for s, w in strategy.target_weights(datetime.now(UTC), ctx).items()
        if s not in OVERNIGHT_MANAGED
    }
...
def main(once: bool = False) -> None:
    ...
    broker.check_setup(blended_weights())  # fail fast on a wrong currency / unresolved ETF
    reconcile(broker)
