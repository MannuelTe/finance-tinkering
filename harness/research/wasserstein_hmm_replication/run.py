"""PURPOSE: run the complete clean-room reproduction and persist audit artifacts.
INPUTS: optional --refresh-data flag.
OUTPUTS: JSON, CSV diagnostics, figures, Markdown report, and a rendered PDF report.
"""
...
from .config import ASSET_LABELS, PUBLISHED, ReplicationConfig
from .data import load_prices
from .report import create_figures, write_markdown, write_pdf
...
def main() -> None:
    ...
    (HERE / "results.json").write_text(json.dumps(result, indent=2, default=float) + "\n")
    ...
    diagnostics.to_csv(HERE / "hmm_daily.csv", index_label="date")
    knn.weights.add_prefix("weight_").assign(
        gross_return=knn.gross_returns, net_return=knn.net_returns, turnover=knn.turnover,
    ).to_csv(HERE / "knn_daily.csv", index_label="date")
    figures = create_figures(HERE, hmm, knn, passive, PUBLISHED, reproduced)
    report = write_markdown(HERE, result)
    pdf = write_pdf(ROOT / "output" / "pdf", result, figures)
