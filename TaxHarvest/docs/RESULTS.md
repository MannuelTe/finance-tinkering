...
| Example | Jurisdiction | Horizon | Target K | α | P(L ≥ K) in-sample | E[L] | Size of S | S / P | Blocked losers |
|---|---|---|---|---|---|---|---|---|---|
| `us_core` | US | 40 d | $4,171 = 23.8% × E[G] | 90% | 90.0% | $9,053 | $59,033 | 20.1% | VOO (IRA contribution buys IVV), NKE (spouse buy), INTC (DRIP) |
| `canada` | CA | 40 d | C$5,815 = 26.8% × E[G] | 90% | 90.1% | C$7,289 | C$12,846 | 5.9% | XIC (spouse buys ZCN, same index), VFV (TFSA DRIP on XUS, same index) |
| `learned_regime` | US | 40 d | $3,456 | 90% | 90.1% | $7,078 | $43,612 | 14.9% | as `us_core` |
| `year_end` | US | 5 d | $9,141 = E[G] (full offset) | 95% | 95.0% | $11,097 | $50,718 | 40.8% | old VTI lot (new VTI lot bought 16 days before the sale) |

E[L] is well above K. With a 40-day horizon the loss distribution is wide: in the US example
the 5th–95th percentile range is $3.0k–$15.1k. Getting the *10% worst case* up to K pushes
the mean to about 2.2 K. Over 5 days (`year_end`) the losses are nearly known, and E[L] is
only 1.2 K.

**US trades** (`figures/us_core/trades.csv`): conditional sells of VEA → IEFA (ρ 1.00),
PFE → XLV (ρ 0.58), DIS → XLC (ρ 0.68), and 68 of 100 XOM → XLE (ρ 0.82). Each comes with
...
is within about 1 point of target from N = 1,000 onwards in every example. At N = 8,000 the
...
**Estimation error.** μ and Σ were redrawn 200 times from their sampling distribution given
5 years of daily data, keeping the plan fixed:

| Example | median P(L ≥ K) | 5th percentile |
|---|---|---|
| us_core | 90.6% | 84.3% |
| canada | 89.9% | 84.5% |
...
for (200k scenarios each; 95% Wilson intervals are ±0.1–0.2 points):

| Evaluated under | us_core | canada | learned_regime | year_end |
|---|---|---|---|---|
| model it was optimised on | 90.2 | 90.2 | 90.2 | 95.1 |
| Student-t, df = 4 (same mean/cov) | 90.5 | 90.6 | 89.8 | 95.5 |
...
drift ±10%/yr and Student-t. The set is deliberately milder than the stresses above, so the
test isn't circular.
...
(calm/crisis, P(stay) = 0.985 / 0.93, crisis vol ×2.3) and learns F_P from that history
alone. BIC picks **k = 2**. Baum–Welch recovers the persistence as **0.985 / 0.939** and the
vol ratio as **2.31** (true value 2.31). The fitted model reproduces the historical left tail,
...
`mc_convergence.gif` (the Monte Carlo estimate of P(L ≥ K) with a Wilson band) and
`frontier_sweep.gif` (how S changes as α rises).
